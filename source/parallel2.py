...
def train_rbf(X_train, X_test, y_train, y_test):
    ...
    for i in range(len(C_range)):
        rbf_parallel=partial(parallel_rbf, i=i, f=f, X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, C_range=C_range, gamma_range=gamma_range)
...
def parallel_rbf(j, i, f, X_train, X_test, y_train, y_test, C_range, gamma_range):
    best_accuracy, best_fbeta, best_recall, best_specificity = 0, 0, 0, 0
    for i in range(len(C_range)):
